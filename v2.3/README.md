...
1. **Place your backend files** (including `main.py`, `preprocess_meds.py`, `maestro_procedimientos.xlsx`, `maestro_medicamentos.xlsx`, and `maestro_diagnosticos.xlsx`) in a folder (e.g., `v2.3/`).
...
3. **Install the required Python packages:**
   ...
   ```
4. **Pre-process Medications:** Before running the backend for the first time, or whenever `maestro_medicamentos.xlsx` is updated, run the pre-processing script:
   ```bash
   python preprocess_meds.py
   ```
   This generates `medications_processed.csv`, which is used by the backend for medication lookups.
...
1. **Navigate to the project's root directory** (the one containing the `v2.3` folder).

2. **Start the server using uvicorn, specifying the app directory:**

   ```bash
   uvicorn main:app --reload --app-dir v2.3
   ```
