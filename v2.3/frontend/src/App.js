...
function App() {
  ...
  // REMOVED diagnosticsMasterName, diagnosticsMasterCode, proceduresMaster states
  ...
  useEffect(() => {
    ...
    axios.get(`${API_BASE}/patients/full/`)
      ...
      .catch(err => console.error("Error fetching patients:", err)); // Add error handling

    // Fetch initial medication list
    axios.get(`${API_BASE}/medications/full/`)
      .then(res => {
        // Correctly map the response which now has label, value, code
        const meds = res.data.map(item => ({
          value: item.value, // Use value from response
          label: item.label, // Use label from response
          code: item.code ? item.code.toString() : "" // Use code from response
        }));
        setMedicationsMaster(meds);
      })
      .catch(err => console.error("Error fetching medications:", err)); // Add error handling
    // REMOVED fetching diagnostics/procedures full lists
    fetchGridData();
  }, []);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      // Re-fetch grid data AND patient list after successful upload
      fetchGridData();
      axios.get(`${API_BASE}/patients/full/`)
           .then(res => setPatientsMaster(res.data.map(p => ({ value: p, label: p }))))
           .catch(err => console.error("Error re-fetching patients after upload:", err));
    } catch (error) {
      console.error("Upload error:", error); // Log specific error
  ...
  // RE-ADD Async load functions for diagnostics and procedures
        ...
        <form key={formKey} onSubmit={handleSubmit}>
          ...
          <div className="form-group diagnostico-group">
            ...
            <div className="diagnostico-fields">
              <AsyncSelect /* Changed back to AsyncSelect */
                cacheOptions
                defaultOptions
                loadOptions={loadDiagnosticNameOptions} // Use load function
              ...
              <AsyncSelect /* Changed back to AsyncSelect */
                cacheOptions
                defaultOptions
                loadOptions={loadDiagnosticCodeOptions} // Use load function
            ...
            {procedimientos.map((item, i) => (
              <div key={`proc-${i}`} className="row-group">
                <AsyncSelect /* Changed back to AsyncSelect */
                  cacheOptions
                  defaultOptions
                  loadOptions={loadProcedureOptions} // Use load function
                  // Value needs to be the full option object for AsyncSelect
                  value={item.name ? { value: item.name, label: `${item.code} - ${item.name}`, code: item.code } : null}
